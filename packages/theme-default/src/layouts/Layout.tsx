...
import { type Feature, Features } from "../components/Features";
...
export const Layout: ParentComponent<LayoutProps> = (props) => {
	...
	const hasSidebar = () =>
		props.config?.theme?.sidebar && props.config.theme.sidebar.length > 0;
	const hasToc = () => toc() && toc().length > 0;

	createEffect(() => {
		const _path = props.currentRoute?.path;
			...
			{isHeroLayout() ? (
				<div class="min-h-screen bg-background">
					...
					<div class="pt-16">
						{/* Hero Section */}
						{frontmatter().hero && (
							<Hero {...(frontmatter().hero as HeroProps)} />
						)}

						{/* Features Section */}
						{(frontmatter().features || []).length > 0 && (
							<Features features={frontmatter().features as Feature[]} />
						)}
							...
							<div class="border-t border-border">
								<div class="px-8 py-12 md:px-12 md:py-16 lg:px-20 lg:py-20">
									<div class="mx-auto max-w-5xl">
										<div class="prose max-w-none">{c()}</div>
									</div>
								</div>
							</div>
						)}
							...
							>
								<div class="flex gap-16 xl:gap-20">
									<main class="flex-1 min-w-0 relative">
										...
										<div class="prose max-w-none xl:max-w-3xl">{c()}</div>
										{docFooter() && (
