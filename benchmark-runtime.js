	...
	for (let i = 0; i < RUNS; i++) {
		...
		const resources = await page.evaluate(() => {
			...
			const jsResources = resources.filter(
				(r) => r.name.endsWith(".js") && r.transferSize > 0,
			);
...
async function runBenchmark() {
	...
	const vitepressResults = await measureSite(
		"http://localhost:4174",
		"VitePress",
	);
	...
	console.log(
		`  VitePress: ${(vitepressResults.bundleSize / 1024).toFixed(2)} KB`,
	);
	console.log(
		`  Difference: ${((leafResults.bundleSize - vitepressResults.bundleSize) / 1024).toFixed(2)} KB\n`,
	);
	...
	console.log(
		`  Difference: ${leafResults.timeToInteractive - vitepressResults.timeToInteractive} ms\n`,
	);
	...
	console.log(
		`  Difference: ${leafResults.firstContentfulPaint - vitepressResults.firstContentfulPaint} ms\n`,
	);

	console.log("Total JS Heap:");
	console.log(
		`  Leaf:      ${(leafResults.totalJSHeap / 1024 / 1024).toFixed(2)} MB`,
	);
	console.log(
		`  VitePress: ${(vitepressResults.totalJSHeap / 1024 / 1024).toFixed(2)} MB`,
	);
	console.log(
		`  Difference: ${((leafResults.totalJSHeap - vitepressResults.totalJSHeap) / 1024 / 1024).toFixed(2)} MB\n`,
	);
	...
	console.log(
		`  Difference: ${leafResults.navigationTiming - vitepressResults.navigationTiming} ms\n`,
	);
}
