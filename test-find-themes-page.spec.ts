		...
		try {
			...
			if (hasThemeContent || hasThemeSwitcher || buttonCount > 0) {
				...
				await page.screenshot({
					path: `test-results/possible-themes-${url.replace(/[/#]/g, "-")}.png`,
					fullPage: true,
				});
				console.log(
					`   📸 已拍攝截圖: test-results/possible-themes-${url.replace(/[/#]/g, "-")}.png`,
				);
			}
			...
			const isHomepage = await page.evaluate(() => {
				...
				return h1?.textContent?.includes("Modern Documentation Framework");
			});
