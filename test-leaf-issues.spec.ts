...
test.describe("Leaf 主題切換 & SSR 問題測試", () => {
	...
	test("主題切換功能測試", async ({ page }) => {
		...
		if (!themeSwitcher) {
			...
			const allButtons = await page.evaluate(() => {
				...
				return buttons.map(
					(b, i) => `${i}: ${b.textContent?.substring(0, 50)}`,
				);
			});
		...
		const fontChanged =
			newFont.includes("Georgia") ||
			cssVariables.fontFamily.includes("Georgia");
	...
	test("主題切換 + 導航綜合測試", async ({ page }) => {
		...
		const themeSwitcher = page
			.locator('button[aria-label="Switch theme"]')
			.first();
		await themeSwitcher.click();

		const businessThemeButton = page
			.locator("button")
