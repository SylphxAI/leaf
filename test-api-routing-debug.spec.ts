...
test('API 路由問題診斷', async ({ page }) => {
  ...
  // 檢查是否被重定向到首頁
  const isHomepage = currentUrl === `${baseUrl}/` || currentUrl === `${baseUrl}`;
  ...
  // 截圖
  await page.screenshot({ path: 'test-results/api-routing-debug.png', fullPage: true });
...
test('檢查正確的 API 文件存在', async ({ page }) => {
  ...
  for (const url of apiUrls) {
    ...
    // 檢查是否正確的 API 內容
    const isCorrectApiContent = h1Text && (
      h1Text.includes('API') ||
