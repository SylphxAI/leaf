---
title: Config API
description: Complete API reference for ReactPress configuration
---

# Config API

Complete reference for ReactPress configuration options.
...
```typescript
function defineConfig(config: ReactPressConfig): ReactPressConfig;
```
...
Custom [remark](https://github.com/remarkjs/remark) plugins.
...
Custom [rehype](https://github.com/rehypejs/rehype) plugins.
...
```typescript
import { loadConfig } from "@sylphx/reactpress";
...
```typescript
function loadConfig(root: string): Promise<ReactPressConfig>;
```
...
```typescript
import { defineConfigWithTheme } from "@sylphx/reactpress";
...
interface Plugin {
  name: string;
  setup?: (config: ReactPressConfig) => void;
...
```typescript
import type { ReactPressConfig, ThemeConfig } from "@sylphx/reactpress";
