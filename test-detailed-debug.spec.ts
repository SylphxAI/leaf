...
test.describe("詳細問題診斷", () => {
	test("主題切換完全失效診斷", async ({ page }) => {
		...
		await page.screenshot({
			path: "test-results/initial-themes-page.png",
			fullPage: true,
		});
		...
		const hasThemeSwitcher =
			pageHtml.includes("Switch theme") ||
		...
		if (buttons.length > 0) {
			...
			// 拍攝點擊後截圖
			await page.screenshot({
				path: "test-results/after-click-theme-button.png",
				fullPage: true,
			});
	...
	test("SSR /guide 路由飛去首頁診斷", async ({ page }) => {
		...
		const hasHomepageContent = pageContent.includes(
			"Leaf - React Documentation Framework",
		);
		...
		// 拍攝截圖
		await page.screenshot({
			path: "test-results/guide-redirect-issue.png",
			fullPage: true,
		});
	...
	test("比較 /guide/installation vs /guide", async ({ page }) => {
		...
		// 拍攝對比截圖
		await page.screenshot({
			path: "test-results/guide-vs-installation.png",
			fullPage: true,
		});
	});
});
