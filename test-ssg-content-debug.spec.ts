...
test('檢查 SSG 生成的 HTML 內容', async ({ page }) => {
  ...
  const hasApiContent = htmlSource.includes('Complete API reference for Leaf') ||
                        htmlSource.includes('Config API') ||
                        htmlSource.includes('Markdown Plugins');

  // 檢查是否包含首頁內容
  const hasHomepageContent = htmlSource.includes('Modern Documentation Framework') &&
  ...
  // 檢查預加載的 frontmatter
  if (preloadData && preloadData.frontmatter) {
    const hasApiFrontmatter = preloadData.frontmatter.title &&
  ...
  // 截圖
  await page.screenshot({ path: 'test-results/ssg-content-debug.png', fullPage: true });
...
test('比較首頁 vs API 頁面的 HTML 源碼', async ({ page }) => {
  ...
  await page.waitForLoadState('networkidle');
  const homepageHtml = await page.evaluate(() => document.documentElement.outerHTML);
  ...
  // 檢查 body 內容的差異
  const homepageBodyContent = homepageHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i)?.[1] || '';
  const apiBodyContent = apiHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i)?.[1] || '';

  const bodyLengthDiff = Math.abs(homepageBodyContent.length - apiBodyContent.length);
